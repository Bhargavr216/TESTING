...
7. If you want to turn plain-English requirements into a configuration file, use the new GenAI helper described below.

## GenAI-backed configuration generation
Create a UTF-8 plain text file (for example `requirements.txt`) that describes the desired setup. Export `OPENAI_API_KEY` so the generator can reach `https://api.openai.com/v1/chat/completions`, and optionally set `OPENAI_MODEL` (defaults to `gpt-4o-mini`). After building the jar you can run:

```sh
java -cp target/automation-1.0-SNAPSHOT.jar com.idea1.automation.genai.GenAiConfigGenerator \
    requirements.txt generated-config.json
```

The utility prompts the model to respond with JSON only, strips markdown fences if any, and writes a pretty-printed configuration file for you to review or wire into your automation pipeline.
