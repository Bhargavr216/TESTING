...
public class Runner {
    ...
    public static void main(String[] args) {
        try {
            ...
            try (Connection conn = DbUtils.getConnection(dbConfig)) {
                ...
                List<ScenarioConsoleResult> scenarioResults = new ArrayList<>();
                ...
                for (EventPayload payload : payloads) {
                    ...
                    ScenarioConsoleResult scenarioResult = new ScenarioConsoleResult(payload.getTest_case_id(), payload.getScenario_name());
                    ...
                    try {
                        ...
                        scenarioFailed = true;
                        scenarioResult.addFailure("Event trigger failed: " + e.getMessage());
                    }
                    ...
                    for (String table : allTables) {
                        ...
                        if (schema == null) {
                            scenarioResult.addFailure(String.format("Schema definition missing for table %s", table));
                            scenarioFailed = true;
                        ...
                        if (!persistenceValid) {
                            scenarioResult.addFailure(String.format("%s persistence violation (expected %s, found %d rows)", table, expectation, rows.size()));
                        ...
                        if ("PERSIST".equals(expectation) && !rows.isEmpty()) {
                            ...
                            for (Map<String, Object> row : rows) {
                                ...
                                if (exp == null) {
                                    scenarioResult.addFailure(String.format("%s: expected persisted row not found", table));
                                    ...
                                    for (String col : schema.getMandatory_columns()) {
                                        ...
                                        if (schema.getJson_columns() != null && schema.getJson_columns().containsKey(col)) {
                                                ...
                                                for (String path : jsonConfig.getRequired()) {
                                                    ...
                                                    for (Map<String, Object> error : jsonErrors) {
                                                        if (path.equals(error.get("path"))) {
                                                            ...
                                                            scenarioResult.addFailure(String.format("%s.%s.%s mismatch: expected %s but got %s", table, col, path, exp_val, act_val));
                                                            break;
                                                        }
                                                    }
                                                    if (!foundError) {
                                            ...
                                            if (!colMatch) {
                                                ...
                                                scenarioFailed = true;
                                                scenarioResult.addFailure(String.format("%s.%s mismatch: expected %s but got %s", table, col, expectedDisplay, actualDisplay));
                                            }
                                ...
                                if (schema.getNull_presence_check() != null) {
                                    ...
                                    for (Map<String, Object> error : nullErrors) {
                                        ...
                                        scenarioResult.addFailure(String.format("%s null-presence violation for %s: expected %s but got %s", table, col, exp_val, act_val));
                                    }
                    ...
                    if (payload.getCheck_exception_persistence() != null) {
                        for (String table : payload.getCheck_exception_persistence()) {
                            ...
                            if (count == 0) {
                                ...
                                scenarioFailed = true;
                                scenarioResult.addFailure(String.format("%s exception persistence check failed (expected NOT NULL rows)", table));
                            }
                        }
                    }
                    ...
                    if (scenarioFailed && !scenarioResult.hasFailures()) {
                        scenarioResult.addFailure("Scenario failed without an explicit failure message; consult the HTML report for details.");
                    }
                ...
                printConsoleSummary(totalCases, passedScenarios, failedScenarios, scenarioResults);
    ...
    private static void printConsoleSummary(int totalCases, int passedScenarios, int failedScenarios, List<ScenarioConsoleResult> results) {
        int skippedScenarios = Math.max(0, totalCases - passedScenarios - failedScenarios);
        System.out.println("\n" + "=".repeat(60));
        System.out.println("TEST EXECUTION SUMMARY");
        System.out.println("-".repeat(60));
        System.out.printf("Total Scenarios : %d%n", totalCases);
        System.out.printf("Passed          : %d%n", passedScenarios);
        System.out.printf("Failed          : %d%n", failedScenarios);
        System.out.printf("Skipped         : %d%n", skippedScenarios);

        if (failedScenarios > 0) {
            System.out.println("\nFailure details (only the failed scenarios):");
            for (ScenarioConsoleResult result : results) {
                if (!result.hasFailures()) continue;
                System.out.println("  " + result.testCaseId + " - " + result.scenarioName);
                for (String detail : result.getFailureMessages()) {
    ...
    private static final class ScenarioConsoleResult {
        private final String testCaseId;
        private final String scenarioName;
        private final List<String> failureMessages = new ArrayList<>();

        private ScenarioConsoleResult(String testCaseId, String scenarioName) {
